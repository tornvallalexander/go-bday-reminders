...
func TestCreateReminderAPI(t *testing.T) {
	user, _ := randomUser(t)
	...
	testCases := []struct {
	...
	}{
		{
			name: "OK",
			body: gin.H{
				"full_name":       reminder.FullName,
				"personal_number": reminder.PersonalNumber,
				"user":            reminder.User,
			...
			buildStubs: func(store *mockdb.MockStore) {
				arg := db.CreateReminderParams{
					...
					PersonalNumber: reminder.PersonalNumber,
					User:           reminder.User,
					PhoneNumber:    reminder.PhoneNumber,
				}
				store.EXPECT().
					CreateReminder(gomock.Any(), gomock.Eq(arg)).
					Times(1).
					Return(reminder, nil)
			},
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
		...
		},
		{
			...
			body: gin.H{
				"full_name":       reminder.FullName,
				"personal_number": reminder.PersonalNumber,
				"user":            "UsernameThatDoesNotExist-123123",
				"phone_number":    reminder.PhoneNumber,
			},
			setupAuth: func(t *testing.T, request *http.Request, tokenMaker token.Maker) {
				addAuthorization(t, request, tokenMaker, authorizationTypeBearer, user.Username, time.Minute)
			...
			buildStubs: func(store *mockdb.MockStore) {
				...
				store.EXPECT().
					CreateReminder(gomock.Any(), gomock.Eq(arg)).
					Times(1).
					Return(db.Reminder{}, &pq.Error{Code: "23503"})
			},
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
		...
		},
		{
			...
			body: gin.H{},
			setupAuth: func(t *testing.T, request *http.Request, tokenMaker token.Maker) {
				addAuthorization(t, request, tokenMaker, "unsupported", reminder.User, time.Minute)
			},
			buildStubs: func(store *mockdb.MockStore) {},
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusUnauthorized, recorder.Code)
		...
		},
		{
			...
			body: gin.H{},
			setupAuth: func(t *testing.T, request *http.Request, tokenMaker token.Maker) {
				addAuthorization(t, request, tokenMaker, "", reminder.User, time.Minute)
			},
			buildStubs: func(store *mockdb.MockStore) {},
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusUnauthorized, recorder.Code)
		...
		},
		{
			...
			body: gin.H{},
			setupAuth: func(t *testing.T, request *http.Request, tokenMaker token.Maker) {
				addAuthorization(t, request, tokenMaker, authorizationTypeBearer, reminder.User, -time.Minute)
			},
			buildStubs: func(store *mockdb.MockStore) {},
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusUnauthorized, recorder.Code)
...
func TestListRemindersAPI(t *testing.T) {
	user, _ := randomUser(t)

	n := 5
	reminders := make([]db.Reminder, n)
	for i := 0; i < n; i++ {
		reminders[i] = randomReminder(user.Username)
	}
	...
	testCases := []struct {
	...
	}{
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				...
				store.EXPECT().
					ListReminders(gomock.Any(), gomock.Eq(arg)).
					Times(1).
					Return(reminders, nil)
			},
			checkResponse: func(recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusOK, recorder.Code)
				requireBodyMatchReminders(t, recorder.Body, reminders)
			},
		},
		{
			name: "NoAuthorization",
...
func requireBodyMatchReminders(t *testing.T, body *bytes.Buffer, reminders []db.Reminder) {
	data, err := ioutil.ReadAll(body)
	require.NoError(t, err)

	var gotReminders []db.Reminder
	err = json.Unmarshal(data, &gotReminders)
	require.NoError(t, err)
